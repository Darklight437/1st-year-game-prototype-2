...
public static class GetArea
{
    ...
    */
    public static List<Tiles> GetAreaOfNotSafeMoveable(Tiles start, int radius, Map map, Unit unit)
    {
    ...
    /*
    * GetAreaOfSafeMoveable
    * public List<Tiles> function (Tiles start, int radius, map)
    * 
    * this function gets all moveable tiles with in the radius given to it
    * around the tile also passed in but ignores trap tiles and damage tiles
    * 
    * @returns List<Tiles> - all movable tiles
    */
    public static List<Tiles> GetAreaOfSafeMoveable(Tiles start, int radius, Map map, Unit unit)
    {
        //set the closed set back to being -1 by default for logic reason purposes
        for (int i = 0; i < closedSet.Length; i++)
        {
        ...
        for (int i = 0; i <= radius; i++)
        {
            ...
            for (int u = 0; u < currSet.Count; u++)
            {
                for (int y = 0; y < currSet[u].tileEdges.Count; y++)
                {
                    if (currSet[u].tileEdges[y].IsPassible(unit) 
                        && FindInContainerClosed(currSet[u].tileEdges[y].indexPos) == false 
                        && currSet[u].tileEdges[y].tileType != eTileType.DAMAGE 
                        && currSet[u].tileEdges[y].tileType != eTileType.PLACABLETRAP)
                    {
                        openSet.Add(currSet[u].tileEdges[y]);
                    }
                }
    ...
    * @returns List<Tiles> - all attackable tiles
    */
    public static List<Tiles> GetAreaOfMeleeAttack(Tiles start, int radius, Map map)
    {
        //set the closed set back to being -1 by default for logic reason purposes
        for (int i = 0; i < closedSet.Length; i++)
        {
        ...
        for (int i = 0; i <= radius; i++)
        {
            ...
            for (int u = 0; u < currSet.Count; u++)
            {
                for (int y = 0; y < currSet[u].tileEdges.Count; y++)
                {
                    if (FindInContainerClosed(currSet[u].tileEdges[y].indexPos) == false && currSet[u].tileEdges[y].IsPassible() == true)
                    {
                        openSet.Add(currSet[u].tileEdges[y]);
                    }
                }
